...
use crate::{data::candidate::get_candidates_data, db::{Campus, Candidate}, util::{log_error, verify_admin_token}};


#[get("/admin/candidate")]
pub async fn get(req: HttpRequest) -> HttpResponse {
      // Get admin or voter token from request cookie
      let admin_or_voter_token = req.cookie("admin_session_token").or(req.cookie("voter_token"));
      ...
      match verify_admin_token(admin_or_voter_token.as_str()).await {
              Ok(_) => (),
              Err(response) => {
                      return response;
              }
