...
use crate::{
    ...
    db::{Campus, Voter},
    rdb::{RedisVoterType, get_voters_data_redis},
    util::{log_error, verify_admin_token},
};
...
pub async fn get(req: HttpRequest, redis_pool: web::Data<RedisPool>) -> HttpResponse {
    ...
    let redis_voter_tokens: Result<HashMap<String, RedisVoterType>, HttpResponse> =
        get_voters_data_redis(&redis_pool).await;
    ...
    let static_voter_tokens: Arc<RwLock<HashMap<String, Vec<Voter>>>> = get_voters_data();
    ...
    for static_voter_token in locked_static_voter_tokens.iter() {
        for voter_data in static_voter_token.1 {
            let result_voters_token_per_campus = result_voters_token.get_mut(&voter_data.campus);
            let result_voters_token_per_campus = match result_voters_token_per_campus {
                Some(data) => data,
                None => {
                    log_error(
                        "GetToken",
                        "There's a voter data in static data where the campus is not in the enum!",
                    );
                    continue;
                }
            };
            result_voters_token_per_campus.insert(static_voter_token.0.clone(), voter_data.token.clone());
        }
    }

    for dynamic_voter_token in redis_voter_tokens.iter() {
        let result_voters_token_by_campus =
            result_voters_token.get_mut(&dynamic_voter_token.1.campus);
        let result_voters_token_by_campus = match result_voters_token_by_campus {
            Some(data) => data,
            None => {
                log_error(
                    "GetToken",
                    "There's a voter data in redis where the campus is not in the enum!",
                );
                continue;
            }
        };

        result_voters_token_by_campus
            .entry(dynamic_voter_token.0.clone())
